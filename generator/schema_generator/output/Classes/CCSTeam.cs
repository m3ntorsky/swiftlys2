using SwiftlyS2.Core.Schemas;
using SwiftlyS2.Shared.Schemas;
using SwiftlyS2.Core.Extensions;

namespace SwiftlyS2.Core.SchemaDefinitions;

internal partial class CCSTeam : CTeam, ICSTeam {

  public CCSTeam(nint handle) : base(handle) {
  }

  public CCSTeam(nint handle, ulong hash, bool isField, bool isNetworked) : base(handle, hash, isField, isNetworked) {
  }

  public ref int LastRecievedShorthandedRoundBonus {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C97862C8DB));
  }
  public ref int ShorthandedRoundBonusStartRound {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C99B1C0796));
  }
  public ref bool Surrendered {
    get => ref _Handle.AsRef<bool>(Schema.GetOffset(0x1CE326C9CFFCED54));
  }
  public ISchemaFixedString TeamMatchStat {
    get => new SchemaFixedString(_Handle + Schema.GetOffset(0x1CE326C9D2C89DC0));
  }
  public ref int NumMapVictories {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C92BC4DE0F));
  }
  public ref int ScoreFirstHalf {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C9E497F5A0));
  }
  public ref int ScoreSecondHalf {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C9A75CAE2C));
  }
  public ref int ScoreOvertime {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C9A22D3CEE));
  }
  public ISchemaFixedString ClanTeamname {
    get => new SchemaFixedString(_Handle + Schema.GetOffset(0x1CE326C946453F76));
  }
  public ref uint ClanID {
    get => ref _Handle.AsRef<uint>(Schema.GetOffset(0x1CE326C90A807BAD));
  }
  public ISchemaFixedString TeamFlagImage {
    get => new SchemaFixedString(_Handle + Schema.GetOffset(0x1CE326C9F7FF31D0));
  }
  public ISchemaFixedString TeamLogoImage {
    get => new SchemaFixedString(_Handle + Schema.GetOffset(0x1CE326C981DF092B));
  }
  public ref float NextResourceTime {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x1CE326C97E7CA6AF));
  }
  public ref int LastUpdateSentAt {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x1CE326C937FCAA0A));
  }

  public void SurrenderedUpdated() {
    Schema.Update(_Handle, 0x1CE326C9CFFCED54);
  }
  public void TeamMatchStatUpdated() {
    Schema.Update(_Handle, 0x1CE326C9D2C89DC0);
  }
  public void NumMapVictoriesUpdated() {
    Schema.Update(_Handle, 0x1CE326C92BC4DE0F);
  }
  public void ScoreFirstHalfUpdated() {
    Schema.Update(_Handle, 0x1CE326C9E497F5A0);
  }
  public void ScoreSecondHalfUpdated() {
    Schema.Update(_Handle, 0x1CE326C9A75CAE2C);
  }
  public void ScoreOvertimeUpdated() {
    Schema.Update(_Handle, 0x1CE326C9A22D3CEE);
  }
  public void ClanTeamnameUpdated() {
    Schema.Update(_Handle, 0x1CE326C946453F76);
  }
  public void ClanIDUpdated() {
    Schema.Update(_Handle, 0x1CE326C90A807BAD);
  }
  public void TeamFlagImageUpdated() {
    Schema.Update(_Handle, 0x1CE326C9F7FF31D0);
  }
  public void TeamLogoImageUpdated() {
    Schema.Update(_Handle, 0x1CE326C981DF092B);
  }
}