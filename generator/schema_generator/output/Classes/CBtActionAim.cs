using SwiftlyS2.Core.Schemas;
using SwiftlyS2.Shared.Schemas;
using SwiftlyS2.Core.Extensions;

namespace SwiftlyS2.Core.SchemaDefinitions;

internal partial class CBtActionAim : CBtNode, IBtActionAim {

  public CBtActionAim(nint handle) : base(handle) {
  }

  public CBtActionAim(nint handle, ulong hash, bool isField, bool isNetworked) : base(handle, hash, isField, isNetworked) {
  }

  public ref CUtlString SensorInputKey {
    get => ref _Handle.AsRef<CUtlString>(Schema.GetOffset(0x43587513D2B0D4C1));
  }
  public ref CUtlString AimReadyKey {
    get => ref _Handle.AsRef<CUtlString>(Schema.GetOffset(0x435875136718A4C7));
  }
  public ref float ZoomCooldownTimestamp {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x43587513F2898D9F));
  }
  public ref bool DoneAiming {
    get => ref _Handle.AsRef<bool>(Schema.GetOffset(0x43587513F2C3ADDE));
  }
  public ref float LerpStartTime {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x43587513B67B575F));
  }
  public ref float NextLookTargetLerpTime {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x43587513768D7F08));
  }
  public ref float PenaltyReductionRatio {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x43587513BB3B4A10));
  }
  public ref QAngle NextLookTarget {
    get => ref _Handle.AsRef<QAngle>(Schema.GetOffset(0x43587513C1D3F3CE));
  }
  public ICountdownTimer AimTimer {
    get => new CountdownTimer(_Handle + Schema.GetOffset(0x4358751330E31F47));
  }
  public ICountdownTimer SniperHoldTimer {
    get => new CountdownTimer(_Handle + Schema.GetOffset(0x43587513B206F2AC));
  }
  public ICountdownTimer FocusIntervalTimer {
    get => new CountdownTimer(_Handle + Schema.GetOffset(0x43587513267063CB));
  }
  public ref bool Acquired {
    get => ref _Handle.AsRef<bool>(Schema.GetOffset(0x435875131D2FE5B5));
  }


}