using SwiftlyS2.Core.Schemas;
using SwiftlyS2.Shared.Schemas;
using SwiftlyS2.Core.Extensions;

namespace SwiftlyS2.Core.SchemaDefinitions;

internal partial class C_OP_RenderAsModels : CParticleFunctionRenderer, IC_OP_RenderAsModels {

  public C_OP_RenderAsModels(nint handle) : base(handle) {
  }

  public C_OP_RenderAsModels(nint handle, ulong hash, bool isField, bool isNetworked) : base(handle, hash, isField, isNetworked) {
  }

  public ref CUtlVector< ModelReference_t > ModelList {
    get => ref _Handle.AsRef<CUtlVector< ModelReference_t >>(Schema.GetOffset(0x634E6CCB05FC11B6));
  }
  public ref float ModelScale {
    get => ref _Handle.AsRef<float>(Schema.GetOffset(0x634E6CCBD28B2146));
  }
  public ref bool FitToModelSize {
    get => ref _Handle.AsRef<bool>(Schema.GetOffset(0x634E6CCBF444BB23));
  }
  public ref bool NonUniformScaling {
    get => ref _Handle.AsRef<bool>(Schema.GetOffset(0x634E6CCBC2ADF0D9));
  }
  public IParticleAttributeIndex_t XAxisScalingAttribute {
    get => new ParticleAttributeIndex_t(_Handle + Schema.GetOffset(0x634E6CCB0E363ADD));
  }
  public IParticleAttributeIndex_t YAxisScalingAttribute {
    get => new ParticleAttributeIndex_t(_Handle + Schema.GetOffset(0x634E6CCBC293ED92));
  }
  public IParticleAttributeIndex_t ZAxisScalingAttribute {
    get => new ParticleAttributeIndex_t(_Handle + Schema.GetOffset(0x634E6CCBC34C4EDF));
  }
  public ref int SizeCullBloat {
    get => ref _Handle.AsRef<int>(Schema.GetOffset(0x634E6CCB5EB61122));
  }


}